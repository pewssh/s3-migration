...
type AwsI interface {
	//Add all functions that would be provided by aws client with necessary arguments
}
...
func GetAwsClient(buckets [][2]string, deleteSource bool, newerThan, olderThan time.Time) (*AwsClient, error) {
	...
	awsClient.newerThan = newerThan
	awsClient.olderThan = olderThan
	awsClient.deleteSource = deleteSource
	return awsClient, nil
}

func splitToBucketAndPrefix(bName string) (bucketName, prefix string, err error) {
	return
}
